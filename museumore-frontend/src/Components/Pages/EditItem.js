  ...
  const handleEditItem = async () => {
    if (title == "") {
      ...
      if (!audioChanged && audio != null) {
        blob = await fetch(audio).then((r) => r.blob({
          'type': 'audio/mpeg'
        }));
        new_audio = new File([blob], audio.substring(audio.lastIndexOf("/") + 1));
      }

      let new_video = extraVideo;
      if (!videoChanged && extraVideo != null) {
        blob = await fetch(extraVideo).then((r) => r.blob({
          'type': 'video/mp4'
        }));
        new_video = new File([blob], extraVideo.substring(extraVideo.lastIndexOf("/") + 1));
      }
  ...
  return (
    <>
      ...
      <div style={{ alignItems: "center", marginTop: "3%" }}>
        ...
        {item.audio && <PlayAudioButton src={audio} />}
        {!item.audio && (
          <AddFileButton
